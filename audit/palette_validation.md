...
校验器：dataviz skill 的 scripts/validate_palette.js（node v24.14.1）
执行时间：2026-08-04T09:51:33Z

## 相邻对（默认；用于堆叠条、并列条、折线、时序带）——前 6 槽
```
...
校验器对 aqua/yellow/magenta 三槽给出 contrast < 3:1 的 WARN，按规则须提供 relief（可见直接标注或表格视图）。本项目两者都提供：每张图对关键系列直接标注数值，且全部图表的底层数值以 CSV 落在 `derived/tables/`，构成 WCAG 洁净的等价表格视图。
