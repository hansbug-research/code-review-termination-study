...
> 基准日 **2026-08-04** ｜ 一手数据 **2,997 个不同 PR** ｜ 图 **18 张** ｜ 可复算表 **12 张** ｜ 机器核对断言 **238 条**（`python3 scripts/verify.py`）｜ 参考文献 **43 条**（全文核对文献 **28 篇**，撤销引用 **6 条**）
...
| 4 | **「带瑕疵合并 + 事后修」的代价有两项，回滚率只是其一。** 回滚率约 0.76%；但「以后再修」的兑现率约在一半上下，缺少具名负责人与到期日时它只是延时生效的技术债 | [§5.3](report.md#53-移出本-pr-的价格revert-率)、[§6.6](report.md#66-出口的定价为什么合并--开个-issue不是免费的)、图 11 |
| 5 | **终止 ≠ 合并，出口是协议的一部分。** 「阻塞」不是统一谓词：P(最终未合并\|曾被阻塞) 在项目间从 8.6% 到 84.3%。出口共六项，换人接手是整合者首选（66.7%），带债合并是这六项策略里最低的（12.8%） | [§2.1](report.md#21-评审循环作为迁移系统)、[§5.5](report.md#55-被正式阻塞的-pr后来怎么了)、[§6.5](report.md#65-第二层出口集合)、图 17 |
| 6 | **失效模式是烂掉，不是死循环。** 被阻塞而死的 PR 中位 77.7 天走到终点（对照组 5.5 天，差 14.2 倍），28.95% 由机器人关闭；真正的振荡只占 2.63% | [§5.5](report.md#55-被正式阻塞的-pr后来怎么了)、图 18 |
| 7 | **加轮次不改变结局。** 33,596 个 agent PR 上，评论数与修订数是仅有的两个不显著变量；显著的是改动规模、文件数与失败的 CI 检查数 | [§7.5](report.md#75-加轮次为什么没用来自-33596-个-agent-pr-的回归证据) |
| 8 | **长 ≠ 不收敛。** 112 天的 PR 只需 1 次批准、0 次阻塞态；3.3 天的 PR 却有 20 次空转、4 次回退 | [§5.4](report.md#54-案例-a一个-112-天但完全收敛的-pr)、[§7.4](report.md#74-案例-bai-写ai-评审全程空转人类仲裁终止)、图 7–9 |
| 9 | **当下 AI 评审的问题主要在集成，不在模型。** 实测机器人 review 正文 56.45% 是机械失败模板，且静默持续了 50 个 PR 无人关闭 | [§7.3](report.md#73-ai-reviewer-的信噪比当下实测)、图 10 |
| 10 | **评判 AI reviewer 的目标线是 64%–68%，不是 100%。** 这是人类 reviewer 评论有用率的实证基线 | [§7.3](report.md#73-ai-reviewer-的信噪比当下实测) |
| 11 | **agent 侧唯一稳健的结论是分层的。** 低成熟度仓库中 agent PR 约 22% 无任何 review 与评论即合并（人类基线 4.2%–4.6%）；成熟仓库样本不足以支持任何跨组比较 | [§7.1](report.md#71-分层结果)–[§7.2](report.md#72-成熟层的聚簇一个足以推翻跨层比较的问题)、图 4–5、15 |
| 12 | **落地不需要造框架。** 出口一项按实测重写：AI 可自主走拆分、换人、关闭与「证据不足自动降级」，具名豁免必须交给有资格承担后果的人。 成品 harness 已把收敛装置产品化；要做的是铺满 verifier、把 AI review 排在人类之前、把预算落到不可绕过的层 | [§8](report.md#8-边界内的解决方案) |
...
- **报告的形式化一度容纳不下自己的结论。** §2.1 原本只把 $B_T=\varnothing$ 定义为终止，而全文主张「不合并也是合法终止」——按原定义那根本不算终止。已把终止态改为 $\lbrace\textsf{MERGE}\rbrace \cup \mathcal{X}$，出口成为形式化的一部分。→ [§2.1](report.md#21-评审循环作为迁移系统)
...
**全部一手数据来自无交付承诺的开源环境**——那里没有人对日期负责，因此「关闭」近乎免费，出口分布不能外推到有交付承诺的场景（[§6.7](report.md#67-交付承诺如何改变出口集合) 讨论了差异，但没有对应的一手数据）。覆盖 5 个大型基础设施仓库与 4 个以 App 身份提交 PR 的 agent，单一时间截面。Cursor 与 Codex 在多数集成模式下以用户身份提交，结构上不可按作者检索，因此 agent 侧样本对当前生态有偏。不做假设检验，只报告描述统计与稳健性诊断。方案章节大量引用官方文档且报告由 Claude Code 生成，存在利益相关性，对冲措施与残余风险见 [§10](report.md#10-局限)。
...
报告所引的 43 条参考文献见 [报告附录 D](report.md#附录-d参考文献)，机器可读版本见 [`references.bib`](references.bib)。
